...
import { putNotificationsRead } from "../../../services/api/notificationApi";
...
const NotificationList = ({ list, isReload, setIsReaload }: INotificationList) => {
  ...
  return (
    <>
      ...
      >
        <DetailNotify isReload={isReload} setIsReload={setIsReaload} setIsModal={setIsModal} data={dataNotifyPending} />
      </BaseModal>
        ...
        {list.map((item) => {
          return (
            <div
              ...
              style={{
                padding: "14px 16px",
                backgroundColor: item.read ? "#fff" : "#f5f8ff",
                borderRadius: 12,
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {item.status === "NEW" ? <Tag color={"gold"}>Đang chờ xử lý</Tag> :
                  item.status === "CONFIRMED" ? <Tag color={"green"}>Đã xác nhận</Tag> :
